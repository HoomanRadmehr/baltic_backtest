...
def gather_prices():
    start_time = int(datetime.datetime.timestamp(datetime.datetime(2023,5,1,0,0,0))*1000)
    end_time = int(datetime.datetime.timestamp(datetime.datetime(2023,8,1,0,0,0))*1000)
    while start_time <= end_time:
        ...
        start_time = int(datetime.datetime.timestamp(datetime.datetime.strptime(list(eth_closes.keys())[-1],'%Y-%m-%d %H:%M:%S')))*1000
...
def resample_timeframe(time_frame):
    ...
    if not path.is_file():
        ...
        with open(f'btc-eth-{time_frame}h.csv','a') as hist_file:        
            closes_btc_eth = {}
            i=0
            for key ,value in btc_closes.items():
                if i%time_frame == 0:
